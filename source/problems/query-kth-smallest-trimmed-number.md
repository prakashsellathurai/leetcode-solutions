...
Try it on <a href='https://leetcode.com/problems/query-kth-smallest-trimmed-number'>leetcode</a>
...
<script type="application/ld+json">
{
  ...
  "mainEntity": {
    ...
    "acceptedAnswer": {
      "@type": "Answer",
      "text": "class Solution:\n    def smallestTrimmedNumbers(self, nums: List[str], queries: List[List[int]]) -> List[int]:\n        results = []\n        for k, trim in queries:\n            # Trim each number to its rightmost 'trim' digits\n            trimmed = [(num[-trim:], i) for i, num in enumerate(nums)]\n            \n            # Sort by the trimmed numbers, breaking ties by index\n            trimmed.sort(key=lambda x: (x[0], x[1]))\n            \n            # Get the index of the k-th smallest trimmed number\n            results.append(trimmed[k-1][1])\n        return results\n\n\n    \n        \n        \n    ",
      "url": "https://prakashsellathurai.com/leetcode-solutions/problems/query-kth-smallest-trimmed-number/",
      "datePublished": "2025-03-03T12:13:04+05:30",
      "upvoteCount": 0,
      "author": {
        "@type": "Person",

...
Try it on <a href='https://leetcode.com/problems/0636-exclusive-time-of-functions'>leetcode</a>
...
class Solution:
    def exclusiveTime(self, n: int, logs: List[str]) -> List[int]:
        res = [0] * n
        cur_func = logs[0].split(":")
        stack = [int(cur_func[0])]
        prev = int(cur_func[2])
        for log in logs[1:]:
            id, marker, time = log.split(":")
            if marker == "start":
                if stack:
...
<script type="application/ld+json">
{
  ...
  "mainEntity": {
    ...
    "acceptedAnswer": {
      "@type": "Answer",
      "text": "class Solution:\n    def exclusiveTime(self, n: int, logs: List[str]) -> List[int]:\n        res = [0] * n\n        cur_func = logs[0].split(\":\")\n        stack = [int(cur_func[0])]\n        prev = int(cur_func[2])\n        for log in logs[1:]:\n            id, marker, time = log.split(\":\")\n            if marker == \"start\":\n                if stack:\n                    res[stack[-1]] += int(time) - prev\n                stack.append(int(id))\n                prev = int(time)\n            else:\n                res[stack[-1]] += int(time) - prev + 1\n                stack.pop()\n                prev = int(time)+1\n        return res\n\n",
      "url": "https://prakashsellathurai.com/leetcode-solutions/problems/0636-exclusive-time-of-functions/",
      "datePublished": "2026-01-13T03:49:18+05:30",
      "upvoteCount": 0,
      "author": {
        "@type": "Person",

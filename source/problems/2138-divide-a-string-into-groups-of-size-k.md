...
Try it on <a href='https://leetcode.com/problems/2138-divide-a-string-into-groups-of-size-k'>leetcode</a>
...
class Solution:
    def divideString(self, s: str, k: int, fill: str) -> List[str]:
        ...
        group = n // k
        lastgroup = n % k
        lastgroupIndex = n - (lastgroup)
        lastword_filler = "".join([fill] * (k - lastgroup))

        for i in range(0, n - k + 1, k):
            res.append(s[i: i + k])

        if lastgroup > 0:
            filler_word = s[lastgroupIndex:] + lastword_filler
            res.append(filler_word)
...
<script type="application/ld+json">
{
  ...
  "mainEntity": {
    ...
    "acceptedAnswer": {
      "@type": "Answer",
      "text": "class Solution:\n    def divideString(self, s: str, k: int, fill: str) -> List[str]:\n        res = []\n        n = len(s)\n        group = n // k\n        lastgroup = n % k\n        lastgroupIndex = n - (lastgroup)\n        lastword_filler = \"\".join([fill] * (k - lastgroup))\n\n        for i in range(0, n - k + 1, k):\n            res.append(s[i: i + k])\n\n        if lastgroup > 0:\n            filler_word = s[lastgroupIndex:] + lastword_filler\n            res.append(filler_word)\n\n        return res\n",
      "url": "https://prakashsellathurai.com/leetcode-solutions/problems/2138-divide-a-string-into-groups-of-size-k/",
